...
void BotWaypointRender(void)
{
	...
	while (i < gWPNum)
	{
		if (gWPArray[i] && gWPArray[i]->inuse)
		{
			...
			if ( level.wptime < level.time )
			{
				level.wpnum++;
				if ( level.wpnum > gWPNum )
				{
					level.wpnum = 0;
				}
				level.wptime = level.time + 2000;

				n = 0;
				while (n < gWPArray[level.wpnum]->neighbornum)
				{
					if (/*gWPArray[i]->neighbors[n].forceJumpTo &&*/ gWPArray[gWPArray[level.wpnum]->neighbors[n].num])
					{
						plum = G_TempEntity( gWPArray[level.wpnum]->origin, EV_BOTSUBWAYPOINT );
						VectorCopy(gWPArray[gWPArray[level.wpnum]->neighbors[n].num]->origin, plum->s.angles);
						plum->r.svFlags |= SVF_BROADCAST;
						plum->s.time = level.wpnum;
//						G_TestLine(gWPArray[i]->origin, gWPArray[gWPArray[i]->neighbors[n].num]->origin, 0x0000ff, 5000);
	...
	if (i >= gWPNum)
	{
		gWPRenderTime = level.time + 1000; //wait a bit after we finish doing the whole trail
		gWPRenderedFrame = 0;
	}
...
checkprint:
	...
	if (gotbestindex && bestindex != gLastPrintedIndex)
	{
		...
		Com_Printf(S_COLOR_YELLOW "Waypoint %i\nFlags - %i (%s) - %i neighbors\nOrigin - (%i %i %i)\n", (int)(gWPArray[bestindex]->index), (int)(gWPArray[bestindex]->flags), flagstr, (int)(gWPArray[bestindex]->neighbornum), (int)(gWPArray[bestindex]->origin[0]), (int)(gWPArray[bestindex]->origin[1]), (int)(gWPArray[bestindex]->origin[2]));
		...
		if (gWPArray[i] && gWPArray[i]->inuse)
		{
			...
			while ( n < gWPNum )
			{
				if ( gWPArray[n] && gWPArray[n]->inuse )
				{
					...
					VectorSubtract(gWPArray[i]->origin, gWPArray[n]->origin, a);
					veclen = VectorLength( a );
					if ( veclen < 800 && veclen < wp[biggest] && (i - n > 2 || n - i > 2) && OrgVisible(gWPArray[i]->origin, gWPArray[n]->origin, 0) )
					{
						int j = 0;

						wp[biggest] = veclen;
						gWPArray[i]->neighbors[biggest].num = n;
						biggest = 0;
						while ( j < 10 )
						{
							if ( wp[j] > wp[biggest] )
